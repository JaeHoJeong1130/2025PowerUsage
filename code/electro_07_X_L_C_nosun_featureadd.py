...
initial_features = [
    ...
    'is_weekend', 'day_of_year', # 시간 관련 피처
    'rolling_3h_temp_mean', 'rolling_24h_temp_mean', 'rolling_24h_temp_max', # 이동 통계 피처
    'lag_1h', 'lag_24h', 'lag_1w' # 시차 변수 (아래 루프에서 생성)
]
...
for building_num in tqdm(range(1, 101), desc="전체 건물 학습 진행"):
    ...
    # [수정] '일조', '일사'는 원본 데이터에만 존재하므로 여기서 제거
    train_building.drop(['일조(hr)', '일사(MJ/m2)'], axis=1, inplace=True, errors='ignore')
    
    # ===================================================================
    # [추가] 시차 변수 (Lag Features) 생성
    # ===================================================================
    train_building['lag_1h'] = train_building['전력소비량(kWh)'].shift(1)
    train_building['lag_24h'] = train_building['전력소비량(kWh)'].shift(24)
    train_building['lag_1w'] = train_building['전력소비량(kWh)'].shift(24 * 7)
    # 결측치를 뒤에서부터 채워줌 (데이터 유출 방지)
    ...
    for model_name in ['xgb', 'lgbm', 'catboost']:
        if model_name == 'lgbm':
            first_pass_model = lgb.LGBMRegressor(**lgbm_params)
        ...
        if model_name == 'xgb':
            final_params = {**grid_search.best_params_, 'n_estimators': 1500}
